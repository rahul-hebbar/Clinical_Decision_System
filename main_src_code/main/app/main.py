...
CORS(app)
app.config['JWT_SECRET_KEY'] = "s[p)Z{mhV;Ll0P9VqLeC1e*?@I$/"

dirname = os.path.dirname(__file__)

# Prepare the labelencoder
with open(os.path.join(dirname,'./dis_class.json'),'r') as file:
...
clf = load(os.path.join(dirname,"./dis_pred.joblib"))

# Load local database
db = TinyDB(os.path.join(dirname,'./user_database.json'))	
